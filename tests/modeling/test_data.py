...
class TestSplitData:
    ...
    def test_split_ratio_approximately_correct(self, feature_dataframe: pd.DataFrame):
        ...
        assert (
            abs(actual_test_ratio - 0.2) <= 0.05
    ...
    def test_different_seeds_produce_different_splits(
        ...
        if len(feature_dataframe) > 20:
            assert not X_train_a.index.equals(
                X_train_b.index
    ...
    def test_target_not_in_feature_matrix(self, feature_dataframe: pd.DataFrame):
        ...
        ), f"Target '{TARGET_COLUMN}' found in X_train"
        assert (
            TARGET_COLUMN not in X_test.columns
        ), f"Target '{TARGET_COLUMN}' found in X_test"
    ...
    def test_class_distribution_preserved(self, feature_dataframe: pd.DataFrame):
        """
        Stratified split should keep the TARGET rate within ±10% of the
        original distribution in both train and test sets.
        """
        original_rate = feature_dataframe[TARGET_COLUMN].mean()
        X_train, X_test, y_train, y_test = split_data(feature_dataframe, stratify=True)
        train_rate = y_train.mean()
        test_rate = y_test.mean()
        assert (
            abs(train_rate - original_rate) <= 0.10
        ), f"Train TARGET rate {train_rate:.2f} deviates from {original_rate:.2f}"
        assert (
            abs(test_rate - original_rate) <= 0.10
        ), f"Test TARGET rate {test_rate:.2f} deviates from {original_rate:.2f}"
    ...
    def test_feature_dtypes_preserved(self, feature_dataframe: pd.DataFrame):
        ...
        X_train, X_test, _, _ = split_data(feature_dataframe)
        original_dtypes = feature_dataframe.drop(columns=[TARGET_COLUMN]).dtypes
