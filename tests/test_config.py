...
class TestBusinessConfig:
    ...
    def test_default_cost_exceeds_revenue(self):
        ...
        assert (
            settings.business.default_cost > settings.business.revenue_per_approval
...
class TestPathConfig:
    ...
    def test_test_paths_use_test_fixtures_dir(self):
        ...
        assert "test" in str(settings.paths.models).lower() or "_fixtures" in str(
            settings.paths.models
        ), f"Test model path does not look like a test path: {settings.paths.models}"

    def test_feature_path_uses_config_path(self):
        """FEATURES_PATH must be sourced from settings.paths.data_features."""
        from src.config import FEATURES_PATH, settings

        assert FEATURES_PATH == settings.paths.data_features
        assert FEATURES_PATH.suffix == ".parquet"
        assert "_fixtures" in str(
            FEATURES_PATH
        ), f"FEATURES_PATH must point to test fixtures in test environment: {FEATURES_PATH}"
...
class TestLoggingConfig:
    ...
    def test_level_is_valid(self):
        ...
        assert hasattr(logging, level_str), f"Invalid logging level: {level_str}"
    ...
    def test_test_env_disables_file_logging(self):
        ...
        assert (
            settings.logging.log_to_file is False
